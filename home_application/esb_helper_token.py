...
def run_fast_execute_script(request, biz_id, script_content, ip_list, username = "admin"):
    """
    快速执行脚本
...
def run_execute_job(request, biz_id, job_id, ip_list, username="admin"):
    """
    快速执行脚本
    :param job_id: 作业模板id
    :param biz_id: 业务id
...
def get_job_instance_log(request, biz_id, job_instance_id, username="admin", count=0):
    """
    获取脚本执行结果
    :param biz_id: 业务ID，
    ...
    if not result["result"]:
        ...
        if count > 5:
            ...
            return []
        time.sleep(2)
        return get_job_instance_log(request, biz_id, job_instance_id, username, count)
...
def run_script_and_get_log_content(request, biz_id, script, ip_list, username):
    """
    执行脚本，并返回执行结果
    """

    f_result = run_fast_execute_script(request, biz_id, script, ip_list, username)

    if f_result["result"]:
        job_instance_id = f_result["data"]
        result = get_job_instance_log(request, biz_id, job_instance_id, username)
        return result[0]["log_content"]

    else:
        return ""

def cc_search_host(request, biz_id, ip_list, username = "admin"):
    '''
    查询主机
    ...
    print 'reponse is: ', response.__str__()
...
def cc_search_set(request, biz_id, username='admin'):
    '''
    查询集群
    :param biz_id: 业务ID，int
    :param ips: 过滤ip
...
def cc_search_biz(request, username='admin'):
    """
    查询业务
    """
...
def cc_get_job_detail(request, biz_id, job_id, username='admin'):
    """
    查询作业模板
    """
...
def cc_fast_push_file(request, biz_id, file_target_path, file_source, target_ip_list, file_source_ip_list, username='admin'):
    """
    文件分发
    """
...
def get_all_user(request, username='admin'):
    """
    获取所有用户信息
    :return:
    """
...
def get_app_info(request, target_app_code, username = "admin"):
    if(target_app_code == ''):
        target_app_code = 'tonychen'
    ...
    kwargs = {
        ...
        'fields': 'creator,introduction,developer'
    }
